...
global usesGUI;
pkg load io
addpath('/Users/anthonyandroulakis/Downloads/spm12-r7487')
addpath('/Users/anthonyandroulakis/Downloads/spm12-r7487/src')
addpath('/Users/anthonyandroulakis/Downloads/NiiStat-master')
if which('NiiStatGUI')
...
%import java.lang.*; %for github
hemiKey = 0;
interhemi = false; %% added by GY at RD's request
statname = '';
%repopath=char(System.getProperty('user.home')); %for github
%checkForUpdate(fileparts(mfilename('fullpath'))); %for github
...
for i = 1: length(modalityIndices) %for each modality
    ...
    for j = 1: length(roiIndices)
        ...
        disp(GUI)
        %j=length(roiIndices)
        %i=length(modalityIndices)
        % #I, Anthony, added these temp changes for debugging purposes. The line below should not be commented out in the real version.
